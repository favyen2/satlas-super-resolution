...
'''
multihead4 is primarily configured by specifying a backbone, optional intermediates, and task-specific heads.
...
class SwinBackbone(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        ...
        elif self.arch == 'swin_seco-ourimages':
            print("Loading in SeCo weights to Swin...")
            swin = torchvision.models.swin_b()
            swin.load_state_dict(torch.load('/multisat/models/seco/models/swin_ourimages_seco.pth'))
            self.backbone = swin
        elif self.arch == 'swin_moco-ourimages':
            print("Loading in MoCo weights to Swin...")
            swin = torchvision.models.swin_b()
            swin.load_state_dict(torch.load('/multisat/models/seco/models/swin_ourimages_moco.pth'))
            self.backbone = swin
        ...
        if self.arch in ['swin_b', 'swin_v2_b', 'swin_seco-ourimages', 'swin_moco-ourimages']:
            self.out_channels = [
                [4, 128],
                [8, 256],
                [16, 512],
...
class ResnetBackbone(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        ...
        # Hardcoded self-supervised method weights
        elif arch == 'resnet50_seco-default':
            resnet = torchvision.models.resnet.resnet50()
            resnet.load_state_dict(torch.load('/multisat/models/seco/models/resnet50_default_local.pth'), strict=False)
            ch = [256, 512, 1024, 2048]
            self.resnet = resnet
        elif arch == 'resnet50_seco-ourimages':
            resnet = torchvision.models.resnet.resnet50()
            resnet.load_state_dict(torch.load('/multisat/models/seco/models/resnet50_ourimages_seco.pth'), strict=False)
            ch = [256, 512, 1024, 2048]
            self.resnet = resnet
...
def rrdb_make_layer(block, n_layers, **kwarg):
    layers = []
...
class RRDBNetBackbone(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        ...
        num_feat = backbone_cfg.get('NumFeat', 64)
        num_grow_ch = backbone_cfg.get('NumGrowCh', 32)
        num_block = backbone_cfg.get('NumBlock', 23)
        print('rrdbnet num_feat={} num_grow_ch={} num_block={}'.format(num_feat, num_grow_ch, num_block))

        self.conv_first = torch.nn.Conv2d(num_channels, num_feat, 3, 1, 1)
        self.body = rrdb_make_layer(RRDB, num_block, num_feat=num_feat, num_grow_ch=num_grow_ch)
...
class RRDBNetTry1Backbone(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        super(RRDBNetTry1Backbone, self).__init__()

        self.num_feat = backbone_cfg.get('NumFeat', 64)
        num_grow_ch = backbone_cfg.get('NumGrowCh', 32)
        num_block = backbone_cfg.get('NumBlock', 23)
        print('rrdbnet num_feat={} num_grow_ch={} num_block={}'.format(self.num_feat, num_grow_ch, num_block))

        self.num_channels = backbone_cfg.get('NumChannels', num_channels)
        self.initial_layers = torch.nn.Sequential(
            torch.nn.Conv2d(3, self.num_feat, kernel_size=3, padding='same'),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(self.num_feat, self.num_feat, kernel_size=3, padding='same'),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(self.num_feat, self.num_feat, kernel_size=1, padding='same'),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(self.num_feat, self.num_feat, kernel_size=1, padding='same'),
            torch.nn.ReLU(inplace=True),
        )
        self.body = rrdb_make_layer(RRDB, num_block, num_feat=self.num_feat, num_grow_ch=num_grow_ch)
        self.conv_body = torch.nn.Conv2d(self.num_feat, self.num_feat, 3, 1, 1)
    ...
    def forward(self, x):
        n_b, n_c, n_h, n_w = x.shape
        n_len = n_c // self.num_channels
        feat = x.reshape(n_b*n_len, self.num_channels, n_h, n_w)
...
backbones['rrdbnet_try1'] = RRDBNetTry1Backbone
...
class AggregationBackbone(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        n_batch = x.shape[0]
        n_images = x.shape[1] // self.image_channels
        n_height = x.shape[2]
        n_width = x.shape[3]
        batched_images = x.reshape(n_batch*n_images, self.image_channels, n_height, n_width)
        all_features = [
            feat_map.reshape(n_batch, n_images, feat_map.shape[1], feat_map.shape[2], feat_map.shape[3])
            for feat_map in self.backbone(batched_images)
        ]
        ...
        for feature_idx in range(len(all_features)):
            ...
            for group in self.groups:
                ...
                for image_idx in group:
                    group_features.append(all_features[feature_idx][:, image_idx, :, :, :])
...
class VideoSwinBackbone(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        super(VideoSwinBackbone, self).__init__()

        # Number of channels in each group.
        self.image_channels = backbone_cfg["ImageChannels"]

        from multisat.models.video_swin import SwinTransformer3D
    ...
    def forward(self, x):
        x = x.reshape(x.shape[0], x.shape[1]//self.image_channels, self.image_channels, x.shape[2], x.shape[3])
        x = x.permute(0, 2, 1, 3, 4)
        outputs = self.backbone(x)
        return [outputs[0][:, :, 0, :, :], outputs[1][:, :, 0, :, :], outputs[3][:, :, 0, :, :]]


backbones['video_swin'] = VideoSwinBackbone

class CustomBackbone1(torch.nn.Module):
    def __init__(self, num_channels, backbone_cfg):
        super(CustomBackbone1, self).__init__()

        num_layers = backbone_cfg['NumLayers']
        num_hidden = backbone_cfg['NumHidden']
        layers = [torch.nn.Sequential(
            torch.nn.Conv2d(num_channels, num_hidden, kernel_size=3, padding='same'),
            torch.nn.ReLU(inplace=True),
        )]
        for _ in range(num_layers):
            layers.append(torch.nn.Sequential(
                torch.nn.Conv2d(num_hidden, num_hidden, kernel_size=3, padding='same'),
                torch.nn.ReLU(inplace=True),
            ))
...
backbones['custom1'] = CustomBackbone1
...
class NoopIntermediate(torch.nn.Module):
    def __init__(self, backbone_channels, module_cfg):
        super(NoopIntermediate, self).__init__()
        self.out_channels = backbone_channels

    def forward(self, x):
        return x

intermediates['noop'] = NoopIntermediate
...
class Model(torch.nn.Module):
    def __init__(self, info):
        ...
        for task_spec, head_config in zip(self.task_specs, self.model_config['Heads']):
            ...
            task = multisat.train.dataset.tasks[task_name]
            head = heads[head_config['Name']](backbone_channels, head_config, task)
            self.heads.append(head)
        self.heads = torch.nn.ModuleList(self.heads)

        self.auto_loss_balance = self.model_config.get('AutoLossBalance', False)
        self.auto_loss_balance_history = self.model_config.get('AutoLossBalanceHistory', 128)
        if self.auto_loss_balance:
            self.recent_losses = [[] for _ in self.heads]
    ...
    def forward(self, image_list, targets=None, selected_task=None):
        ...
        for task_idx, head in enumerate(self.heads):
            ...
            cur_outputs, cur_loss = head(image_list, features, cur_targets)
            outputs.append(cur_outputs)
            if cur_loss is not None:
                losses.append(cur_loss)

        if len(losses) > 0:
            if 'LossWeights' in self.model_config:
                losses = [losses[i] * self.model_config['LossWeights'][i] for i in range(len(losses))]

            if self.auto_loss_balance:
                for i, loss in enumerate(losses):
                    self.recent_losses[i].append(loss.item())
                    while len(self.recent_losses) > self.auto_loss_balance_history:
                        self.recent_losses[i].pop(0)

                averages = [max(np.mean(l), 1e-3) for l in self.recent_losses]
                losses = [losses[i] / averages[i] for i in range(len(losses))]
